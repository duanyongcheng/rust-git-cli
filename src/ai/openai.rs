...
impl OpenAIClient {
    ...
    pub async fn generate_review_report(
        ...
        for attempt in 0..max_attempts {
            let mut messages = vec![Message {
                role: "system".to_string(),
                content: "You are a helpful assistant that reviews code changes in JSON format. Reply with exactly one valid JSON object.".to_string(),
            }];

            if attempt > 0 {
                messages.push(Message {
                    role: "system".to_string(),
                    content: "Your previous answer was truncated. Send the complete JSON object this time, avoid any commentary or markdown fences.".to_string(),
                });
            }
            ...
            let report = match serde_json::from_str::<ReviewReport>(clean_content) {
                ...
                Err(primary_err) => {
                    ...
                    if let (Some(start), Some(end)) = (start_idx, end_idx) {
                        let json_str = &clean_content[start..end];
                        serde_json::from_str::<ReviewReport>(json_str).with_context(|| {
                            format!(
                                "Failed to parse extracted JSON from OpenAI response: {}",
                                primary_err
                            )
                        ...
                        return Err(anyhow::anyhow!(
                            "Failed to parse review report from OpenAI response: {}",
                            primary_err
                        ));
                    }
                }
        ...
        anyhow::bail!("Failed to obtain a valid review report from OpenAI after multiple attempts");
    }
